...
func _tween_zoom(new_zoom: Vector2):
	tween.interpolate_property(self, "zoom", self.zoom, new_zoom, TWEEN_SPEED, Tween.TRANS_QUAD, Tween.EASE_IN)
	tween.start()
	pass
...
func _tween_position(new_pos: Vector2): #NOTE: New_pos must be in global coordinate. Otherwise will be thrown way off
	...
	tween.interpolate_property(self, "position", self.position, target_pos, TWEEN_SPEED, Tween.TRANS_QUAD, Tween.EASE_IN)
	tween.start()
	pass
