...
#Public Functions
func set_pick_color(new_color, internal = false): #internal is just to check if it's an internal call
	color = new_color
	if internal:
...
func get_presets() -> PoolColorArray:
	var preset_array = PoolColorArray(preset_colors)
	return preset_array
...
func _on_AddButton_pressed(): #Largely redundant but gives the AddButton some use
